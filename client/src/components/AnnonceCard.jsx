...
export default function AnnonceCard({ annonce, onFavori, isFavori }) {
  ...
  // Icônes minimalistes
  const icons = {
    surface: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2" ry="2"/><line x1="9" y1="3" x2="9" y2="21"/></svg>,
    type: <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>
  };
      ...
      <div className="card-image-wrapper">
        ...
        {user?.role === "etudiant" && (
          <button 
            ...
            onClick={(e) => { 
              e.stopPropagation(); // Empêche d'ouvrir l'annonce
        ...
        <div className={`card-status-tag tag-${annonce.statut}`}>
          {annonce.statut === 'en_attente' ? 'En attente' : annonce.statut}
        </div>
      </div>

      <div className="card-content">
        <div className="card-meta">📍 {annonce.ville}</div>
        <h3 className="card-title">{annonce.titre}</h3>

        <div className="card-info-row">
          <div className="info-item">{icons.type} <span>{annonce.typeLogement}</span></div>
          {annonce.surface && <div className="info-item">{icons.surface} <span>{annonce.surface} m²</span></div>}
        </div>
        ...
        <div className="card-footer">
          ...
          {/* Le bouton profite du onClick de la div parente */}
